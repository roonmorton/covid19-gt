...
Municipios.getCasesInfo = function (result) {
    sql.query(`
            SELECT TC.id_caso as id_case,TEP.id_estado,TP.id_persona as id_person,
            TP.id_genero as id_gender,
            TD.id_departamento as id_department,
            TP.nombres AS name,
            TP.apellidos as lastname, 
            TG.nombre_genero as gender, 
            TP.edad as age,
            TEP.nombre_estado as status,
            TD.nombre_departamento as department_name, 
            TC.descripcion_caso as description,
            TC.direccion_caso as address ,
            ifnull(DATE_FORMAT(TC.fecha_contagio,'%Y-%m-%d'),'NO DISPONIBLE') as contagion_date,
            ifnull(DATE_FORMAT(TC.fecha_recuperacion,'%Y-%m-%d') ,'NO DISPONIBLE')as recovery_date 
            FROM tbl_casos TC 
            INNER JOIN tbl_personas TP 
            ON TP.id_persona=TC.id_persona 
            INNER JOIN tbl_departamentos TD 
            ON TD.id_departamento=TC.id_departamento 
            LEFT JOIN tbl_generos TG 
            ON TG.id_genero=TP.id_genero 
            INNER JOIN tbl_estados_pacientes TEP 
            ON TEP.id_estado=TC.id_estado 
...
Municipios.getCaseById = function (caseId, result) {
    sql.query(`
            SELECT 
            TC.id_caso as id_case,
            TEP.id_estado,
            TP.id_persona as id_person,
            TP.id_genero as id_gender,
            TD.id_departamento as id_department,
            TP.nombres AS name,
            TP.apellidos as lastname, 
            ...
            TC.descripcion_caso as description,
            TC.direccion_caso as address ,
            ifnull(DATE_FORMAT(TC.fecha_contagio,'%Y-%m-%dT%h:%i:%S.000Z'),'NO DISPONIBLE') as contagion_date,
            ifnull(DATE_FORMAT(TC.fecha_recuperacion,'%Y-%m-%dT%h:%i:%S.000Z') ,'NO DISPONIBLE')as recovery_date 
            FROM tbl_casos TC 
            INNER JOIN tbl_personas TP 
            ON TP.id_persona=TC.id_persona 
            INNER JOIN tbl_departamentos TD 
