...
Municipios.getByGenderAge = function(result) {
    sql.query("SELECT TG.nombre_genero as bars,(case  when TG.id_genero=1 then 'pink'  when TG.id_genero=2 then '#76A7FA'  when TG.id_genero=3 then 'gray' end) as color,count(case when TP.edad>0 and TP.edad<11 then 1 else null end) as \'0-10\', count(case when TP.edad>10 and TP.edad<21 then 1 else null end) as \'11-20\', count(case when TP.edad>20 and TP.edad<31 then 1 else null end) as \'21-30\', count(case when TP.edad>30 and TP.edad<41 then 1 else null end) as \'31-40\', count(case when TP.edad>40 and TP.edad<51 then 1 else null end) as \'41-50\', count(case when TP.edad>50 and TP.edad<61 then 1 else null end) as \'51-60\', count(case when TP.edad>60 and TP.edad<71 then 1 else null end) as \'61-70\', count(case when TP.edad>70 and TP.edad<81 then 1 else null end) as \'71-80\', count(case when TP.edad>80 and TP.edad<91 then 1 else null end) as \'81-90\', count(case when TP.edad>90 then 1 else null end) as \'mas de 90\' FROM tbl_personas TP INNER JOIN tbl_generos TG ON TP.id_genero=TG.id_genero GROUP BY TG.nombre_genero ORDER BY TG.nombre_genero desc", function(err, res) {
        if (err) {
            console.log("error: ", err);
            result(null, err);
        } else {
